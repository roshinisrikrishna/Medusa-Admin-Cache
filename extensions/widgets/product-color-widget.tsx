...
const ProductWidget = ({ product, notify }: ProductDetailsWidgetProps) => {
  ...
  const fetchImageUrls = async () => {
    const urls = await Promise.all(colorValues.map(async (color) => {
      ...
      try {
        console.log('modifiedOptionId', modifiedOptionId)
        const response = await axios.get(`http://localhost:9000/store/colorImage`, {
          params: { option_id: modifiedOptionId }
        });
        console.log('response get', response)
        console.log('response.data.data[0].thumbnail', response.data.data[0].thumbnail)
        return { color, url: response.data.data[0].thumbnail || "" };
      } catch (error) {
...
const handleImageUrlChange = (color, url) => {
    setImageUrls(prev => ({ ...prev, [color]: url }));
};


 // Function to handle upload button click
 const handleUpload = (color) => {
    const imageUrl = imageUrls[color];
    const originalOptionId = product.options.find(option => option.title.toLowerCase() === color.toLowerCase())?.id;
    ...
    console.log('option_id', colorOptionId)
    const requestBody = {
          ...
          <table style={{ borderCollapse: 'collapse', width: '100%' }}>
            ...
            <tbody>
              {colorValues.map((color, index) => (
                <tr key={index}>
                  <td style={{ borderBottom: '1px solid gray', color:"rgba(0,0,0,0.8)",padding:"3%" }}>{color}</td>
                  <td style={{ borderBottom: '1px solid gray', padding:"3%", justifyContent:"center",alignItems:"center",display:"flex" }}>
                    {thumbnailUrls[color] ? (
                      <img
                        src={thumbnailUrls[color]}
                        alt={`${color}`}
                        style={{ width: '25%', height: 'auto' }} // Adjust size as needed
                      />
                    ):(
                        <img
                        ...
                        alt={`${color}`}
                        style={{ width: '40%', height: 'auto' }} // Adjust size as needed
                      />
                
                    )}
                  </td>
                  <td style={{ borderBottom: '1px solid gray', padding:"3%" }}>
                    {thumbnailUrls[color] && !changeImageClicked[color] ? (
                      <Button variant="outlined" onClick={() => handleChangeImageClick(color)}>
                        Change Image
                      </Button>
                    ) : (
                      <TextField
                        id={`outlined-${color}`}
                        label={`Change image for ${color}`}
                        variant="outlined"
                        value={imageUrls[color] || ""}
                        size='small'
                  ...
                  <td style={{ borderBottom: '1px solid gray', padding:"3%" }}>
                    <Button variant="outlined" color='secondary' endIcon={<SendIcon />} onClick={() => handleUpload(color)} style={{padding:"3% 6%"}}>Upload</Button>
                  </td>
                </tr>
              ))}
            </tbody>
